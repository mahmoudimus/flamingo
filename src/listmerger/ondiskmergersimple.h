/*
  $Id: ondiskmergersimple.h 5149 2010-03-24 23:37:18Z abehm $
...
merge_Impl(const Query& query,
  ...
  if(numberFilters == 1 && this->singleFilterOpt && arrays.size() > 1) {
	...
	for(unsigned j = 0; j < temp.size(); j++)
	  if(this->charsumFilter->passesFilter(this->queryCharsumStats, &this->charsumStats[temp.at(j)], (unsigned)query.threshold)) results.push_back(temp.at(j));
      }
      else {
