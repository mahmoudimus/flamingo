...
class DivideSkipMerger : public ListsMerger<DivideSkipMerger<InvList>, InvList> {
 private:  
  ...
  void mergeSkip(const vector<Array<unsigned>*> &arrays,
  ...
  void detectDuplicateLists(vector<Array<unsigned>*> &arrays,
  ...
  void getQueryStats(const vector<Array<unsigned>*> &arrays,
		     const unsigned threshold,
		     float& shortListsVar,
 ...
 public:   
  ...
  void fillEstimationParams(const vector<vector<Array<unsigned>* >* >& arrays,
			    const vector<unsigned>& thresholds);  

  float getEstimatedQueryTime(const vector<Array<unsigned>*>& arrays,
...
void 
DivideSkipMerger<InvList>::
mergeSkip(const vector<Array<unsigned>*> &arrays,
	  const unsigned threshold,
	  const unsigned longLists,
	  vector<unsigned> &results,
	  vector<unsigned> &counters) {
...
void 
DivideSkipMerger<InvList>::
getQueryStats(const vector<Array<unsigned>*> &arrays,
	      const unsigned threshold,
	      float& shortListsVar,
	      float& shortListsTime,
	      float& longListsVar,
...
void 
DivideSkipMerger<InvList>::
fillEstimationParams(const vector<vector<Array<unsigned>* >* >& arrays,
		     const vector<unsigned>& thresholds) {
  ...
  for(unsigned i = 0; i < arrays.size(); i++) {
    vector<Array<unsigned>*>* currentArrs = arrays.at(i);
...
float
DivideSkipMerger<InvList>::
getEstimatedQueryTime(const vector<Array<unsigned>*>& arrays,
		      const unsigned threshold) const {
...
void  
DivideSkipMerger<InvList>::
detectDuplicateLists(vector<Array<unsigned>*> &arrays,
		     vector<Array<unsigned>*> &newArrays,
		     vector<unsigned> &newWeights) {
  ...
  for(unsigned i = 0 ; i < arrays.size(); i++) {   
	...
	Array<unsigned> *iArray = arrays.at(j);
