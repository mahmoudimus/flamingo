/*
  $Id: ftindexercombinelists.h 5146 2010-03-24 23:05:57Z abehm $
...
lshGreedyTh(float ratio) {
  ...
  for(i = 0; i < 18; i++) {
    ...
    index.buildIndex();    
    GramMap& gramMap = index.filterTreeRoot->children.at(0).node->gramMap;
    CandiSet cps;
...
chooseUnionLists() {  
  ...
  this->originalIndexSize = 
    FilterTreeNode<InvList>::getSubTreeInvertedListsSize(this->globalIndex->filterTreeRoot->children.at(0).node);
  
  float unionTh = this->lshGreedyTh(1.0f - this->reductionRatio);
  
  // TODO do something about the method
  unsigned method = 3;
  GramMap& gramMap = this->globalIndex->filterTreeRoot->children.at(0).node->gramMap;
  ...
  this->copyGramMapSkeleton(this->globalIndex->filterTreeRoot->children.at(0).node->gramMap,
...
buildMergeEstimationModel(const vector<string>& sampleWorkload) {
  vector<vector<Array<unsigned>*>*> allLists;
  vector<unsigned> allThresholds;
  FilterTreeNode<>* leaf = this->globalIndex->filterTreeRoot->children.at(0).node;

  // gather all lists from all queries
  for(unsigned i = 0; i < sampleWorkload.size(); i++) {
    const string& query = sampleWorkload.at(i);
    vector<Array<unsigned>*>* invertedLists = new vector<Array<unsigned>*>();
    vector<unsigned> gids;
  ...
  for(unsigned i = 0; i < allLists.size(); i++) {
    vector<Array<unsigned>*>* invertedLists = allLists.at(i);
...
initEstimations() {  	   
  ...
  for(i = 0; i < sampleWorkload.size(); i++) {
    ...
    for(j = 0; j < invertedLists.size(); j++) {
      Array<unsigned>* a = invertedLists.at(j);
      for(k = 0; k < a->size(); k++)
	q.scanCounts[a->at(k)]++;      
    }
...
greedyIteration(GramMap& gramMap, float unionTh, unsigned k, CandiSet& cps) {
  ...
  GramMap& queriesMap = this->queryIndex->filterTreeRoot->children.at(0).node->gramMap;
...
unionEstimation(GramMap& gramMap, float unionTh, unsigned gida, unsigned gidb,
  ...
  Array<unsigned> &a = gla->find()->list;
  Array<unsigned> &b = glb->find()->list;
  
  // gida, gidb might be missing for query list, if so use dummy instead
  GramMap& queriesMap = this->queryIndex->filterTreeRoot->children.at(0).node->gramMap;  
  Array<unsigned> dummy;
  Array<unsigned> *qa = &dummy, *qb = &dummy;
