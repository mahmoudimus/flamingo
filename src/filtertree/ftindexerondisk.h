/*
  $Id: ftindexerondisk.h 5146 2010-03-24 23:05:57Z abehm $
...
template<class StringContainer = StringContainerRM, class InvList = Array<unsigned> >
class FtIndexerOnDisk 
...
protected:
  ...
  FilterTreeNode<InvList>* findHomeLeafNode(const string s);
...
public:  
  ...
  FtIndexerOnDisk(StringContainer* strContainer, CharsumFilterOptions csfOpts = CSF_REG) 
    : SuperClass(strContainer, csfOpts), indexSize(0) {
  ...
  FtIndexerOnDisk(StringContainer* strContainer, 
		  ...
		  CharsumFilterOptions csfOpts = CSF_REG,
		  bool scatteredOrg = false) 
    : SuperClass(strContainer, gramGen, maxStrLen, ftFanout, csfOpts),
...
// functions that depend on the type of the inverted list element, e.g. unsigned or PosID
...
public:
  ...
  static void updateLeafOffsets(GramListOnDiskPair& currentPair,
	...
	sortedSubListSize = 1;
	sortedSubListOffset = gl->startOffset + (streampos)(j * InvList::elementSize());
      }
...
// specialization for Array<PosID>
template<>
class InvListSpecializer<Array<PosID> > {
public:
  typedef Array<PosID> InvList;
  typedef tr1::unordered_map<unsigned, GramList<InvList>* > GramMap;
  typedef pair<unsigned, GramListOnDisk<Array<PosID> >*> GramListOnDiskPair;
  
  static vector<unsigned>* stringid2leafid;
  
  static int cmp(const void* a, const void* b) {
    const PosID* x = static_cast<const PosID*>(a);
    const PosID* y = static_cast<const PosID*>(b);    
    if(stringid2leafid->at(x->id) > stringid2leafid->at(y->id) || x->id > y->id) return 1;
    if(stringid2leafid->at(x->id) < stringid2leafid->at(y->id) || x->id < y->id) return -1;
    if(stringid2leafid->at(x->id) == stringid2leafid->at(y->id) && x->id == y->id) return (int)x->pos - (int)y->pos;
    return 0;
  }
  ...
  static void updateLeafOffsets(GramListOnDiskPair& currentPair,
    ...
    // need to update offsets of filtertree leaves
    PosID* list = (PosID*)writeMe.first;
    unsigned sortedSubListSize = 1;
    streampos sortedSubListOffset = gl->startOffset;
    unsigned prevLeaf = stringid2leafid->at(list[0].id); // set to first value
    for(unsigned j = 1; j < gl->listSize; j++) {
      unsigned currentLeaf = stringid2leafid->at(list[j].id);
      if( currentLeaf != prevLeaf ) {
	// update the grammap in appropriate leaf
	GramMap& gramMap = leaves.at(prevLeaf)->gramMap;
	if(gramMap.find(currentPair.first) == gramMap.end())
...
saveLeavesRec(FilterTreeNode<InvList>* node, ofstream& fpOut) {
  ...
  if(node->isLeaf) {
    if(node->distinctStringIDs) {
      GramListOnDisk<Array<unsigned> >* gl = dynamic_cast<GramListOnDisk<Array<unsigned> >*>(node->distinctStringIDs);
...
loadLeavesRec(FilterTreeNode<InvList>* node, ifstream& fpIn) {
  if(node->isLeaf) {
    ...
    if(distinctStrings > 0) {
      ...
      node->distinctStringIDs = new GramListOnDisk<Array<unsigned> >(distinctStrings, startOffset);
    }
...
buildIndex_Impl() {
  ...
  for(unsigned i = 0; i < this->strContainer->size(); i++) {
    ...
    // gather information for charsum filtering
    ...
    unsigned bytes = this->gramGen->getNumGrams(currentString) * InvList::elementSize();
  ...
  if(this->filterTypes.size() > 0) {    
    ...
    mergeRuns(numberRuns, this->filterTreeRoot->children.at(0).node->gramMap, fpRunFile, fpIndex);
    fpIndex.close();
    fpRunFile.close();
...
checkCorrectness(FtIndexerMem<StringContainer, InvList>* memIndexer, const string& invListsFileName) {
  ...
  for(unsigned leaf = 0; leaf < memLeaves.size(); leaf++) {
	...
	Array<unsigned>* memArr = memGl->getArray();
	...
	return false;
      ...
      unsigned bytes = diskList->listSize * InvList::elementSize();
      char* buffer = new char[bytes];
      fpIn.read(buffer, bytes);      
	...
	return false;
      ...
      typename InvList::elementType* diskArr = (typename InvList::elementType*) buffer;
...
flushRun(FtIndexerMem<StringContainerVector, InvList>& memIndexer, fstream& fpOut) {
  typename FilterTreeNode<InvList>::GramMap& gramMap = memIndexer.filterTreeRoot->children.at(0).node->gramMap;
  typename FilterTreeNode<InvList>::GramMap::iterator iter;
  ...
  for(iter = gramMap.begin(); iter != gramMap.end(); iter++) {
    ...
    fpOut.write((const char*)&size, sizeof(unsigned));
    const char* listData = (const char*)arr->begin();
    fpOut.write(listData, size * InvList::elementSize());    
...
mergeRuns(unsigned numberRuns, GramMap& targetMap, fstream& fpIn, fstream& fpOut) {
  ...
  for(iter = totalGramListSizes.begin(); iter != totalGramListSizes.end(); iter++) {
    unsigned bytes = iter->second * InvList::elementSize();
  ...
  for(unsigned run = 0; run < numberRuns; run++) {
    ...
    for(unsigned list = 0; list < numberPartialLists; list++) {
      ...
      unsigned bytes = allLists[list].numberElements * InvList::elementSize();
	...
	nextOffset += totalGramListSizes[gramCode] * InvList::elementSize();
	targetMap[gramCode] = new GramListOnDisk<InvList>(totalGramListSizes[gramCode], writeOffset);
      ...
      unsigned bytes = pList->numberElements * InvList::elementSize();
    ...
    for(unsigned list = 0; list < completeLists.size(); list++) {
      ...
      unsigned bytes = cList->numberElements * InvList::elementSize();
...
reorganizeIndex(GramMap& sourceMap, vector<FilterTreeNode<InvList>* >& leaves, fstream& fpIndex) {
  ...
  InvListSpecializer<Array<unsigned> >::stringid2leafid = &stringid2leafid;
  qsort(distinctIDs, 
	this->strContainer->size(), 
	sizeof(unsigned), 
	InvListSpecializer<Array<unsigned> >::cmp);
  ...
  for(unsigned j = 1; j < this->strContainer->size(); j++) {
	...
	new GramListOnDisk<Array<unsigned> >(sortedSubListSize, sortedSubListOffset);
      prevLeaf = currentLeaf;
      sortedSubListSize = 1;
  ...
  for(unsigned i = 0; i < sortedGramLists.size(); i++) {
    ...
    writeMe.second = gl->listSize * InvList::elementSize();
...
FtIndexerOnDisk<StringContainer, InvList>::
findHomeLeafNode(const string s) {  
