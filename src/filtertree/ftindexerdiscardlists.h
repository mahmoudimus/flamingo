/*
  $Id: ftindexerdiscardlists.h 5144 2010-03-24 22:06:23Z abehm $
...
typedef struct {
  ...
  Array<unsigned*>* nonZeroScanCounts;
  unsigned panicCost;
...
protected:
  ...
  void getFilterTreeLeaves(const FtIndexerMem<>& ftIndexer,
			   FilterTreeNode<Array<unsigned> >* node, 
			   const string& query, 
			   const vector<AbstractFilter*>& filterTypes, 
			   const unsigned filterId, 
			   vector<FilterTreeNode<Array<unsigned> >* >& leaves);

  unsigned initScanCountArray(const vector<Array<unsigned>*> &arrays,
...
getFilterTreeLeaves(const FtIndexerMem<>& ftIndexer,
  ...
  if(node->isLeaf) leaves.push_back((FilterTreeNode<>*)node);
  else {
...
unsigned 
FtIndexerDiscardListsCostAbs<FtIndexerDiscardListsConcrete>::
initScanCountArray(const vector<Array<unsigned>*> &arrays,
		   const unsigned threshold,
		   const unsigned arraySize,
		   unsigned scanCountArray[]) {
...
fillQueryInfo(const FtIndexerMem<>& sampleIndexer,
  ...
  for(unsigned i = 0; i < queriesSampleSet.size(); i++) {   
    ...
    queryInfo[i].nonZeroScanCounts = new Array<unsigned*>[nLeaves];

    queryInfo[i].panicCost = 0;    
    queryInfo[i].isPanic = false;
    queryInfo[i].originalCandidates = 0;    
    for(unsigned j = 0; j < nLeaves; j++) {
      FilterTreeNode<Array<unsigned> >* leaf = queryInfo[i].affectedLeaves.at(j);
      ...
      vector<Array<unsigned> *> invertedLists;
	...
	Array<unsigned*>* nonZeroScanCounts = &queryInfo[i].nonZeroScanCounts[j];
	for(unsigned z = 0; z < dictSize; z++)
	  if(arr[z] > 0) nonZeroScanCounts->append( &arr[z] ); 	
      }
...
freeQueryInfo(QueryInfo queryInfo[], const unsigned size) {
  for(unsigned i = 0; i < size; i++) {
    ...
    Array<unsigned*>* nonZeroScanCounts = queryInfo[i].nonZeroScanCounts;
...
protected:  
  ...
  unsigned calculateNewCandidates(const string& query,
				  ...
				  const unsigned arraySize,
				  const Array<unsigned*>& nonZeroScanCounts,
				  const Array<unsigned>* affectedStrings,
...
estimateAvgMergeTime(const vector<string>& dictSampleSet,
  ...
  DivideSkipMerger<InvList> merger;
...
simulateHole(const bool makePermanent,
  ...
  if(queriesMap.find(hole) != queriesMap.end()) {	
    ...
    Array<unsigned>* changedQueries = gramList->getArray();
	...
	for(unsigned j = 0; j < affectedLeaves->size(); j++) {
	  ...
	  Array<unsigned>* invertedList = NULL;
...
chooseHoleGrams_Cost() {  
  ...
  GramMap& queriesMap = queriesIndexer.filterTreeRoot->children.at(0).node->gramMap;  
