...
bool
Rtree::GenerateMBRfromNode(struct Node * node, struct Rect * the_mbr_rect)
{
    // true if all ok, false otherwise (like if the node has
    // no boxes.
    assert(NULL != node);
    assert(NULL != the_mbr_rect);
    bool res = false;
...
void
Rtree::print_rect(ostream & os, struct Rect & rect, bool add_endl)
{
    os << " [";
    int i;
...
void
Rtree::CreateSpheres(struct Node *n)
{ // this function will create all the spheres
    int i;
...
int
Rtree::RTreeInsertRect(struct Rect *R, int Tid)
{
...
int
Rtree::RTreeInsertRect(struct Rect *R, int Tid, struct Node **Root, int Level)
{
    struct Rect *r = R;
    int tid = Tid;
...
static void
RTreeFreeListNode(struct ListNode *p)
{
    //free(p);
    delete(p);
}
...
int
Rtree::RTreeSearch(struct Rect *R, vector<uintptr_t> &answers)
{
    long disk_acc[10];
...
int
Rtree::SearchJoinOpen(Rtree * rt, DistanceFunction * df,
                      float * max_join_dist,
...
{ // total distance computations
    ...
    int i;
    for (i = 0; i < join_disk_acc_length1; i++) {
        join_disk_acc1[i] = 0;
    }
    for (i = 0; i < join_disk_acc_length2; i++) {
        join_disk_acc2[i] = 0;
    }
    for (i = 0; i < join_cpu_comp_length; i++) {
        join_cpu_comp [i] = 0;
    }
...
void
Rtree::join_aux_node_node(JoinElement::JoinTerm * first, JoinElement::JoinTerm * second,
                          JoinElement & j_el, bool swap, int exploration_model)
{
    // this compares internal nodes in the tree
    assert(!first->is_data);
    assert(!second->is_data); // none is a point
...
void
Rtree::join_aux_node_leaf(JoinElement::JoinTerm * first, JoinElement::JoinTerm * second,
                          JoinElement & j_el, bool swap, int exploration_model)
{
    ...
    for (int i = 0; i < first->the_node->count; i++) {
        for (int j = 0; j < second->the_node->count; j++) {
            ...
            j_el_aux.second.the_mbr = second->the_node->branch[j].rect;
            j_el_aux.second.the_id = static_cast<int>(reinterpret_cast<uintptr_t>(second->the_node->branch[j].child));
...
void
Rtree::join_aux_leaf_leaf(JoinElement::JoinTerm * first, JoinElement::JoinTerm * second,
                          JoinElement & j_el, bool swap, int exploration_model)
{
    // compares two leaves
    assert(!first->is_data);
    ...
    for (int i = 0; i < first->the_node->count; i++) {
        for (int j = 0; j < second->the_node->count; j++) {
            ...
            j_el_aux.first.the_mbr = first->the_node->branch[i].rect;
            j_el_aux.first.the_id = static_cast<int>(reinterpret_cast<uintptr_t>(first->the_node->branch[i].child));
...
void
Rtree::join_aux_leaf_point(JoinElement::JoinTerm * first, JoinElement::JoinTerm * second,
                           JoinElement & j_el, bool swap, int exploration_model)
{
    // compares a leaf to a point (first is leaf, second is point)
    assert(!first->is_data);
    assert(second->is_data); // only second is a point
...
void
Rtree::join_aux_node_point(JoinElement::JoinTerm * first, JoinElement::JoinTerm * second,
                           JoinElement & j_el, bool swap, int exploration_model)
{
...
int
Rtree::SearchJoinNext(float & distance, join_pair_result & item)
{
    assert(join_active_query);
    assert(NULL != join_dist_func);
...
int
Rtree::SearchJoinNewIteration(DistanceFunction * df_new, int & cpu_distance_comps,
                              int & nodes_copied, int reconstruct_model)
{
    assert(join_active_query);
    assert(NULL != join_dist_func); // ensure correct state
    return 1; // all ok
}
